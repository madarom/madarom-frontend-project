function initInvoices() {
      ...
      paginated.forEach(order => {
        ...
        container.innerHTML += `
          <div class="relative bg-white p-5 rounded-xl shadow-md">
            ...
            <button onclick="viewInvoice('${order.id}')" class="mt-4 flex items-center justify-center gap-2 btn-default text-sm px-4 py-2 rounded-full w-full">
    ...
    async function fetchInvoices() {
        ...
        try {
            ...
            allOrders = data.filter(q => q.quote && q.quote.status?.toLowerCase() === "facturation");
