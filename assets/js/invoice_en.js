...
function renderQuotes(data) {
  tbody.innerHTML = '';
  if (data.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" class="px-6 py-4 text-center text-gray-500">No invoices found.</td></tr>`;
...
async function fetchUserQuotes() {
  ...
  try {
    ...
    allQuotes = data.filter(quote => quote.quote.status?.toLowerCase() === "facturation");
    applyFilters();
  } catch (error) {
